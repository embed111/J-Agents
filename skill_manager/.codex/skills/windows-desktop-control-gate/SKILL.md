...
1. 盘点目标仓库，确认它是 Windows 桌面自动化场景，并且允许新增 `scripts/` 与 `workspace_state/`。
...
6. 先跑 `powershell -ExecutionPolicy Bypass -File scripts/test-control-request.ps1`，再做一次真实安全预览。
...
- `scripts/test-control-request.ps1`：mock 测试脚本，验证请求/授权/释放/过期清理。
...
2. `powershell -ExecutionPolicy Bypass -File <target-workspace>\\scripts\\test-control-request.ps1`
3. 在目标仓库做一次真实预览，只展示审批窗与操作中提示条，不直接发送业务输入。
4. 确认目标仓库生成 `workspace_state/control/control-log.jsonl`，且日志包含 `control_requested` 与 `control_released`。
...
```bash
powershell -ExecutionPolicy Bypass -File C:\work\my-automation\scripts\test-control-request.ps1
```
